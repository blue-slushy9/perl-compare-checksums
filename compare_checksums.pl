...
sub calculate_checksum {
  ...
  # Open file for reading; 'die' ends script execution in the case that the 
  # file couldn't be opened; '$!' contains the error message returned by the OS
  ...
  $md5->addfile($fh);
  close($fh);
  # Return checksum in HEX format to be captured by below variables
